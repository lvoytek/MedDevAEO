...
void web_listen_for_clients() {
  ...
  if (!client.connectSSL(host, httpPort)) {//check to see if client is connected
    ...
    if (PingResult < 0) {//try to reconnect to wifi if the ping result gives an error code
      //server.end();
      WiFi.disconnect();
      delay(3000);
      setup_wifi();
  ...
  while((num_bytes = client.available())){//collect response from client
    //Serial.print("Number of bytes  = ");
    //Serial.println(num_bytes);
